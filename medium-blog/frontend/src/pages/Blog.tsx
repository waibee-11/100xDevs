    ...
    <div className="h-full">
        ...
        <div className="grid grid-cols-12 my-12 mx-12 h-full">
            <div className="col-span-8 w-full flex flex-col gap-7">
                <div className="text-6xl font-semibold font-lora">{blog.title}</div>
                {blog.coverImage === "" ? null : <div className="mt-4 "><img className="rounded-lg w-full object-fill" src={blog.coverImage || ""} alt="" /></div>}
                <div className="text-2xl whitespace-pre-wrap leading-tight font-lora">{blog.content}</div>
            </div>
            <div className="col-span-4 ml-20 flex flex-col gap-10">
                <div className="border-b border-slate-500 w-full pb-10">
                    <div className="flex items-center gap-5">
                        <div className="flex border-4 p-1 border-black w-20 h-20 rounded-full text-5xl bg-black text-white font-bold justify-center items-center">{blog.author.name[0]}</div>
                        <div className="flex flex-col">
                            <div className="text-xl">Author:</div>
                            <div className="text-3xl font-semibold font-lora">{blog.author.name || "Anonymous"}</div>
                        </div>
                    </div>
                </div>
